...
fun run_update :: "run \<Rightarrow> constr \<Rightarrow> run" ("_ \<langle> _ \<rangle>") where
  ...
  | "\<rho> \<langle> K \<Down> n @ \<lfloor> \<tau> \<rfloor> \<rangle> = Abs_run (\<lambda>n' K'. if K = K' \<and> n = n' then (hamlet ((Rep_run \<rho>) n K), \<tau>) else (Rep_run \<rho>) n' K')"
  | "\<rho> \<langle> K \<Down> n @ \<lfloor> \<tau>\<^sub>v\<^sub>a\<^sub>r(K', n') \<oplus> \<tau> \<rfloor> \<rangle> =
     Abs_run (\<lambda>n'' K''. if K = K'' \<and> n = n'' then (hamlet ((Rep_run \<rho>) n K), time ((Rep_run \<rho>) n' K') + \<tau>) else (Rep_run \<rho>) n'' K'')"
  | "\<rho> \<langle> \<tau>\<^sub>v\<^sub>a\<^sub>r(K\<^sub>1, n\<^sub>1) \<doteq> \<alpha> * \<tau>\<^sub>v\<^sub>a\<^sub>r(K\<^sub>2, n\<^sub>2) + \<beta> \<rangle> =
     Abs_run (if time ((Rep_run \<rho>) n\<^sub>1 K\<^sub>1) = undefined \<and> time ((Rep_run \<rho>) n\<^sub>2 K\<^sub>2) \<noteq> undefined
      then (\<lambda>n K. if K = K\<^sub>1 \<and> n = n\<^sub>1 then (hamlet ((Rep_run \<rho>) n K), \<alpha> * time ((Rep_run \<rho>) n\<^sub>2 K\<^sub>2) + \<beta>) else (Rep_run \<rho>) n K)
      else if time ((Rep_run \<rho>) n\<^sub>2 K\<^sub>2) = undefined \<and> time ((Rep_run \<rho>) n\<^sub>1 K\<^sub>1) \<noteq> undefined
           then (\<lambda>n K. if K = K\<^sub>2 \<and> n = n\<^sub>2 then (hamlet ((Rep_run \<rho>) n K), undefined (* (time (\<rho> n\<^sub>1 K\<^sub>1) - \<beta>) / \<alpha> *)) else (Rep_run \<rho>) n K)
           else (Rep_run \<rho>) (* MAYBE NOT A GOOD CHOICE *)
     )"
