...
# +  description {* Denotational and operational semantics for TESL *}/g' src/ROOT
	cd src && $(ISABELLE) build -c -d . -o browser_info -v TESL_Language
	mv "`$(ISABELLE) getenv -b ISABELLE_BROWSER_INFO`/Unsorted/TESL_Language" .
	cp docs/FirstExample.png TESL_Language/
	rm -fr docs
	mv TESL_Language docs
